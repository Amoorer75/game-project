...
let input = getInput(`Will the next card be higher(h) or lower(l) than your card`)
...
// 6. If input equals l, check and see if it's a positive number.
  ...
  input = getInput('You have to guess eather h or l. this round you will not receive points')
  return false
}
...
function playGame(){
 ...
 // 8. Inside the while loop, create a conditional statement. If the outcome of guess is true, increment the score by 1, congratulate the user, and tell them their score. If it's false, tell them they were wrong and got no points.
    if (guess() == false){
    score ++ 
    console.log(`congratulations ${playerName}, and score is ${score}`)  
   }else{
    console.log(`Sorry ${playerName} you were wrong and score is still ${score}`)
   }
  ...
  return (deck.length == 0? `you are out of cards, you lost.`:`you have won`)
